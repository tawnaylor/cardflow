...
test('One Piece flow adds card', async ({ page }) => {
  await page.goto('/cardflow/form.html?brand=onepiece');
  await page.waitForSelector('#brand');
  await page.evaluate(() => { const el = document.querySelector('#brand'); if (el) { el.value = 'onepiece'; el.dispatchEvent(new Event('change')); } });
