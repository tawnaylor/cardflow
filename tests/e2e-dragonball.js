...
(async () => {
  ...
  console.log('Opening form page (Dragon Ball)...');
  ...
  await page.fill('#cardName', 'E2E DragonBall Card');
  await page.fill('#marketValue', '6.66');
  await page.fill('#cardDetails', 'E2E test Dragon Ball details');
  ...
  if (cards.length === 0 || gridCount === 0) {
    console.error('Dragon Ball Test failed: no card stored or rendered');
    await browser.close();
    process.exit(2);
  }

  await browser.close();
  console.log('Dragon Ball E2E test completed successfully.');
})();