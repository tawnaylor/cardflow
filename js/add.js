...
function validate(formEl) {
  ...
  if (!formEl.elements.name.value.trim() || formEl.elements.name.value.trim().length < 2) problems.push("Card name is required.");
  if (!formEl.elements.series.value.trim()) problems.push("Series is required.");
  if (!formEl.elements.expansion.value.trim()) problems.push("Series expansion is required.");
  if (!formEl.elements.rarity.value) problems.push("Rarity is required.");
  if (!CARD_NUMBER_PATTERN.test(formEl.elements.number.value.trim())) problems.push("Invalid card number.");
  return problems;
}
...
form.addEventListener("submit", async (e) => {
  ...
  const payload = {
    // Addition: Ensures numeric ID and numeric Quantity
    binderId: Number(binderSelect.value), 
  ...
  setStatus(result.merged ? `Merged! Now x${result.card.qty}.` : `Added "${result.card.name}"!`);
  form.reset();
...
seedBtn.addEventListener("click", () => {
  if (!binderSelect.value) {
    setStatus("Select a binder first!");
    return;
  }
  const bId = Number(binderSelect.value); 
  const demo = [
    { name:"Pikachu", series:"Scarlet & Violet", expansion:"Paldea Evolved", rarity:"Rare", number:"1", qty:2, imageDataUrl:"", binderId: bId },
    { name:"Charizard", series:"Scarlet & Violet", expansion:"Obsidian Flames", rarity:"Ultra Rare", number:"2", qty:2, imageDataUrl:"", binderId: bId }
  ];
  for (const c of demo) upsertCard(c);
  setStatus("Demo seeded!");
});
